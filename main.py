...
def main() -> None:
    ...
    if args.render_from_metrics is not None:
        ...
        print(f"Rendered visuals from metrics: {args.render_from_metrics}")
        print(f"Visual report: {Path(visualization_config.output_dir) / 'report.html'}")
        return
    ...
    print(f"Final unique strategies: {final.num_unique_strategies}")
    print(f"Final population size: {final.total_population_size}")
    print(f"Winning DNA: {final.dominant_dna}")
    print(f"Winning group size: {final.dominant_group_size}")
    print(f"Final cooperation rate: {final.overall_cooperation_rate:.3f}")
    print(f"Final dominant share: {final.dominant_strategy_share:.3f}")
